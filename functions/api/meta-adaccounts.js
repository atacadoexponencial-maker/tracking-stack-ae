...
export async function onRequestGet({ request, env }) {
  ...
  const r = await fetch(`https://graph.facebook.com/v25.0/me/adaccounts?fields=id,name,account_status&limit=50&access_token=${token}`);
  const corpo = await r.json();
  if (!r.ok) return json({ error: 'adaccounts falhou', detalhe: corpo }, 502);

  const contas = [];
  for (const c of corpo.data || []) {
    const teste = await fetch(`https://graph.facebook.com/v25.0/${c.id}/insights?fields=spend&date_preset=yesterday&access_token=${token}`);
    contas.push({ id: c.id, nome: c.name, status: c.account_status, insights_ok: teste.ok });
  }
  return json({ contas });
}
